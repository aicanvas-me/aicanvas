...
//
// Andromeda PRO's foundation loop. It reads Pro's injected tokens and rides
// Pro's `--at-*` theme channel, so it belongs to Pro's pages alone. Andromeda
// Legacy keeps its own FoundationLoop, which reads Legacy's committed tokens
// and its own theme channel; sharing one component made the Legacy landing and
// the homepage render Pro's palette. Renders in Andromeda's own visual language
...
const at = (name: string, value: string) => `var(--at-${name}, ${value})`
...
function swatchRow(label: string, family: string) {
  const stops = [100, 200, 300, 400, 500]
  return {
    label,
    items: stops.map((stop) => ({
      key: `${family}-${stop}`,
      value: (C as unknown as Record<string, Record<string, string>>)[family][stop],
    })),
...
function FSwatchRow({
  ...
  row: { label: string; items: { key: string; value: string }[]; vars: Record<string, string> }
}) {
...
export function FoundationLoopPro() {
